...
DEFAULT_MODEL  = r'SG161222/Realistic_Vision_V4.0_noVAE'
...
from include.Portrait.Fuser.Portrait_Fuser import Fuser_ResponseStreamer
...
class Copilot_InputPanel(wx.Panel, NewChat, GetClassName, Base_InputPanel_Portrait_SD):
    ...
    def __init__(self, parent, tab_id):
        ...
        self.askButton = wx.Button(self, label='Generate', size=(70, 25))
        ...
        #askSizer.Add(self.img_source_dropdown, 0, wx.ALIGN_CENTER)
    ...
    def OnImgSourceChange(self, event):

        # This method will be called when the selection changes
        selected_value = self.img_source_dropdown.GetValue()
        print(f"Selected img_source: {selected_value}")
        chat = apc.chats[self.tab_id]
        chat.img_source = selected_value 
        ...
        try:
            ...
            if not question:
                ...
                if 'system_prompt' not in chat:
                    ...
                    num_op=str(chat.num_of_images)
                    chat.system_prompt=evaluate(all_system_templates[chat.workspace].Copilot[system], dict2(num_of_images=num_op))
                    pub.sendMessage('set_system_prompt', message=chat.system_prompt, tab_id=self.tab_id)
                    #print(system_prompt)

                threading.Thread(target=self.stream_response, args=(prompt, chatHistory, self.tab_id, image_path)).start()
        except Exception as e:
