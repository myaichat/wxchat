        ...
        async with websockets.connect(ws_url) as websocket:
            ...
            observer_js = f'''
                (async function() {{
                    ...
                    // Much improved completion detection - more conservative approach
                    function checkIfComplete(element, text) {{
                        ...
                        // More conservative: require BOTH definitive indicator AND natural ending
                        return hasDefinitiveIndicator && endsNaturally;
                    }}
                                ...
                                mutation.addedNodes.forEach(function(node) {{
                                    if (node.nodeType === Node.ELEMENT_NODE && !responseFound) {{
                                        ...
                                        if (isAssistantMessage(node)) {{
                                            ...
                                            // Much more conservative completion check
                                            ...
                                            const completionCheck = setInterval(() => {{
                                                ...
                                                console.log(`COMPLETION_DEBUG: length=${{currentContent.length}}, stable=${{stableTextCount}}, timeSinceUpdate=${{timeSinceLastUpdate}}ms`);
                                                
                                                // More sophisticated stability check
                                                if (currentContent.length === lastSeenLength && currentContent.length > 100) {{
                                                    // Text length hasn't changed and we have substantial content
                                                    if (timeSinceLastUpdate > 3000) {{ // 3 seconds since last update
                                                        stableTextCount++;
                                                        console.log(`COMPLETION_CHECK: stable=${{stableTextCount}}, checking completion indicators`);
                                                        
                                                        // Much higher threshold: 30 checks (15 seconds) of stability
                                                        if (stableTextCount >= 30) {{
                                                            // Additional check: does it really look complete?
                                                            if (checkIfComplete(responseElement, currentContent)) {{
                                                                ...
                                                                // Reset counter if it doesn't look complete
                                                                stableTextCount = 0;
                                                                console.log("COMPLETION_DEBUG: Reset counter - not actually complete");
                                                            }}
                                                        }}
                                                    }} else {{
