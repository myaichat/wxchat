...
class Base_InputPanel_Gryphe_MythoMax(Base_InputPanel):
    ...
    def AddButtons_Level_2(self, v_sizer):
        if 1: #second row
            ...
            #repetition_penalty
            self.frequency_penalty_dropdown = wx.ComboBox(self, choices=['-2.0',  '-1.0',  '0.0','0.1', '0.2', '0.5',  '1.0','1.1', '1.5', '2.0'], style=wx.CB_READONLY)
            self.frequency_penalty_dropdown.SetValue('0.1')  # Default value
            self.frequency_penalty_dropdown.Bind(wx.EVT_COMBOBOX, self.OnRepetitionPenaltyChange)
            chat.frequency_penalty = float(self.frequency_penalty_dropdown.GetValue()  )             

            self.presence_penalty_dropdown = wx.ComboBox(self, choices=['-2.0',  '-1.0',  '0.0', '0.3', '0.8',  '1.0', '1.3', '1.8',  '2.0'], style=wx.CB_READONLY)
            self.presence_penalty_dropdown.SetValue('0.3')  # Default value
            self.presence_penalty_dropdown.Bind(wx.EVT_COMBOBOX, self.OnLengthPenaltyChange)
            chat.presence_penalty = float(self.presence_penalty_dropdown.GetValue()  )    
...
"""   
            ...
            sizer_0 = wx.BoxSizer(wx.VERTICAL)
            dos = wx.StaticText(self, label=" freq_penalty")
            dos.html_content ="""<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Frequency Penalty Ranges Explained</title>
</head>
<body>
    <h1>Frequency Penalty Ranges Explained</h1>
    
    <h2>Frequency Penalty Range Effects:</h2>
    
    <h3>Low Frequency Penalty (0.0 to 0.1):</h3>
    <ul>
        <li><strong>Behavior:</strong> The model slightly discourages the use of common words, leading to minor variations in word choice while maintaining natural language flow.</li>
        <li><strong>Output:</strong> The text will have minor lexical diversity with high coherence, producing natural-sounding and fluent sentences.</li>
    ...
    <h3>Moderate Frequency Penalty (0.2 to 0.5):</h3>
    <ul>
        <li><strong>Behavior:</strong> The model moderately discourages the use of common words, encouraging more varied and creative word choices.</li>
        <li><strong>Output:</strong> The text will be more diverse and creative, with less repetition of common words and potentially more interesting language.</li>
        <li><strong>Use Case:</strong> Suitable for creative writing, brainstorming, and content that benefits from a higher level of lexical diversity and variation.</li>
    </ul>
    
    <h3>High Frequency Penalty (0.6 to 1.0):</h3>
    <ul>
        <li><strong>Behavior:</strong> The model heavily discourages the use of common words, promoting maximum lexical diversity and creativity.</li>
        <li><strong>Output:</strong> The text will be highly diverse and creative, with minimal repetition of common words, but may risk losing some coherence.</li>
    ...
    <h3>Very High Frequency Penalty (>1.0):</h3>
    <ul>
        <li><strong>Behavior:</strong> The model severely discourages the use of common words, pushing for extreme lexical variation and novelty.</li>
        <li><strong>Output:</strong> The text will be extremely varied and creative, with very low repetition of common words, but may become difficult to follow or understand.</li>
    ...
    <pre>
{
  ...
  "top_k": 50
    ...
    <p><strong>Output:</strong> Text with minor lexical diversity and high coherence.</p>
    
    <h3>Moderate Frequency Penalty:</h3>
    <pre>
{
  ...
  "top_k": 50
    ...
    <p><strong>Output:</strong> More diverse and creative text with less repetition of common words.</p>
    
    <h3>High Frequency Penalty:</h3>
    <pre>
{
  ...
  "top_k": 50
    ...
    <p><strong>Output:</strong> Highly diverse and creative text with minimal repetition of common words.</p>
    
    <h3>Very High Frequency Penalty (Maximum Creativity):</h3>
    <pre>
{
  ...
  "top_k": 50
    ...
    <p><strong>Output:</strong> Extremely varied and creative text with very low repetition of common words.</p>
...
"""   
            ...
            sizer_0.Add(dos, 0, wx.ALIGN_CENTER)
            sizer_0.Add(self.frequency_penalty_dropdown, 0, wx.ALIGN_CENTER)
            h_sizer_1.Add(sizer_0, 0, wx.ALIGN_CENTER)

            sizer_0 = wx.BoxSizer(wx.VERTICAL)
            dos = wx.StaticText(self, label=" presence_penalty")
            dos.html_content ="""
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Presence Penalty Ranges Explained</title>
</head>
<body>
    <h1>Presence Penalty Ranges Explained</h1>
    
    <h2>Presence Penalty Range Effects:</h2>
    
    <h3>Low Presence Penalty (0.1 to 0.5):</h3>
    <ul>
    ...
    <h3>Moderate Presence Penalty (0.6 to 1.0):</h3>
    <ul>
    ...
    <h3>High Presence Penalty (1.1 to 1.5):</h3>
    <ul>
    ...
    <h3>Very High Presence Penalty (>1.5):</h3>
    <ul>
    ...
    <pre>
{
    ...
    <h3>Moderate Presence Penalty:</h3>
    <pre>
{
    ...
    <pre>
{
    ...
    <h3>Very High Presence Penalty:</h3>
    <pre>
{
...
>"""   
            ...
            dos.Bind(wx.EVT_LEFT_DOWN, lambda event: self.OnClickDos(event))
            sizer_0.Add(dos, 0, wx.ALIGN_CENTER)
            sizer_0.Add(self.presence_penalty_dropdown, 0, wx.ALIGN_CENTER)
            h_sizer_1.Add(sizer_0, 0, wx.ALIGN_CENTER)
...
enhancing the quality and coherence of the generated text.
    ...
    def OnRepetitionPenaltyChange(self, event):
        # Get the selected do_sample value
        selected_repetition_penalty = self.frequency_penalty_dropdown.GetValue()

        # Print the selected model
        chat = apc.chats[self.tab_id]
        chat.frequency_penalty = float(selected_repetition_penalty )
    ...
    def OnLengthPenaltyChange(self, event):
        # Get the selected do_sample value
        selected_length_penalty = self.presence_penalty_dropdown.GetValue()
    ...
    def RestoreQuestionForTabId(self, tab_id):
        ...
        if message in self.tabs:
            ...
            if chat.get('repetition_penalty', None):
                self.frequency_penalty_dropdown.SetValue(str(chat.repetition_penalty))
            else:
                chat.frequency_penalty = float(self.frequency_penalty_dropdown.GetValue())


            if chat.get('length_penalty', None):
                self.presence_penalty_dropdown.SetValue(str(chat.length_penalty))
            else:
                chat.presence_penalty = float(self.presence_penalty_dropdown.GetValue())
