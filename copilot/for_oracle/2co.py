...
with open('template/Python/Gpt4/Chat/default.Python.Gpt4.Chat.yaml', 'r') as stream:
    chat_templates = d2d2(yaml.safe_load(stream))

templates.update(chat_templates['templates'])

with open('template/Python/Gpt4/Copilot/default.Python.Gpt4.Copilot.yaml', 'r') as stream:
    copilot_templates = d2d2(yaml.safe_load(stream))
#pp(copilot_templates)
...
if 0:
    ...
    templates.Copilot.update(
    SYSTEM = """You are a chatbot that assists with anwering questions about  code included  or  adding new features 
    ...
    [NEW CODE LINES]
    """,
    SYSTEM_ASK_ABOUT_CODE = """You are a chatbot that assists with anwering questions about 
    code included or  adding new features
    """,
    FIX_CODE='''
...
class NewChatDialog(wx.Dialog):
    def __init__(self, *args, **kwargs):
        ...
        self.system.SetValue(templates.Chat [default_chat_template] )
...
class Gpt4_Copilot_DisplayPanel(wx.Panel):
    def __init__(self, parent, tab_id):
        ...
        self.notebook_panel=notebook_panel = MyNotebookCodePanel(self.copilot_splitter)
...
class Gpt4_Chat_InputPanel(wx.Panel, NewChat,GetClassName, Base_InputPanel):
    ...
    def SetChatDefaults(self, tab_id):
        ...
        if tab_id ==self.tab_id:
            ...
            self.tabs[self.tab_id]=dict(q="List new Python 3 syntax features")
            chatHistory[self.tab_id]= [{"role": "system", "content": templates.Chat[default_chat_template]}]
...
class Gpt4_Copilot_InputPanel(wx.Panel, NewChat, GetClassName, Base_InputPanel):
    def __init__(self, parent, tab_id):
        ...
        chatHistory[self.tab_id]=[]
        chatHistory[self.tab_id]= [{"role": "system", "content": templates.Copilot[default_copilot_template]}]
