...
class MainFrame(wx.Frame):
    def __init__(self, size=(800, 600)):
        ...
        if 0:
            ...
            self.answer_output.InsertColumn(2, 'Elapsed Sec', width=100)
            self.answer_output.InsertColumn(3, 'Text', width=500)
        if 1:
            self.answer_output = wx.dataview.DataViewListCtrl(panel, wx.ID_ANY, style=wx.BORDER_THEME | dv.DV_ROW_LINES | dv.DV_VERT_RULES)
            self.answer_output.AppendTextColumn('Line Num')  # Add columns as needed
            self.answer_output.AppendTextColumn('Elapsed Sec')
            self.answer_output.AppendTextColumn('Model')
            if 1:
                self.answer_output.AppendTextColumn('Text',  mode=dv.DATAVIEW_CELL_INERT,
                                   width=300)

                # Set the minimum size of the column
                #column.SetMinSize(300)

                # Add the column to the DataViewListCtrl
                #self.answer_output.AppendTextColumn(column)
            else:
                self.answer_output.AppendTextColumn('Text')
            self.Bind(dv.EVT_DATAVIEW_COLUMN_HEADER_CLICK, self.OnResize, self.answer_output)
    ...
    def OnResize(self, event):
            ...
            self.dvlc.Columns[column].SetWidth(wx.DV_AUTO_SIZE)

    def on_resize(self, event):
        # Update the size of the ListCtrl to match the size of the frame
        # Get the DataViewCtrl from the DataViewListCtrl
        data_view_ctrl = self.answer_output.GetMainWindow()

        # Calculate the total width of the other columns
        total_width = sum(data_view_ctrl.GetColumn(i).GetWidth() for i in range(self.answer_output.GetColumnCount() - 1))

        # Set the width of the last column to the remaining width of the ListCtrl
        last_column = data_view_ctrl.GetColumn(self.answer_output.GetColumnCount() - 1)
        last_column.SetWidth(self.answer_output.GetSize()[0] - total_width)
    ...
    def add_row(self,  text):
        ...
        elapsed_time = time.time() - self.start_time
        num_items = self.answer_output.GetItemCount()
        row=[str(num_items+1), str(elapsed_time), self.model_selector.GetValue(), 'test\ntest']
        #self.answer_output.AppendItem([num_items, elapsed_time, self.model_selector.GetValue(), text])
        self.answer_output.AppendItem(row)
        #self.answer_output.SetColumnWidth(0, 200)

    def append_to_row(self,  text):
        return
        # Get the current value of the cell
        row = self.answer_output.GetItemCount()
        column=3
        current_value = self.answer_output.GetValue(row, column)
        pp(current_value)
        # Append the new text to the current value
        new_value = str(current_value) + text
        
        # Set the new value of the cell
        self.answer_output.SetValue(new_value, row, column)

    def _append_to_row(self, text):
