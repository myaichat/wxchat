...
from include.Prompt.Base.Base_InputPanel_Google_Gemma import Base_InputPanel_Google_Gemma as Base_InputPanel_1
...
class NoHist_4bit_ResponseStreamer:
    ...
    def get_model(self, model_id):
        ...
        if model_id not in self.model:
            print("Loading new model 11", model_id)
...
class NoHist_4bit_pChat_ResponseStreamer:
    subscribed=False
    def __init__(self):
        # Set your OpenAI API key here
        self.model={}
    ...
    def get_model(self, model_id):
        dtype = torch.float16
        if model_id not in self.model:
            print("Loading new model", model_id)
            from transformers import AutoTokenizer, AutoModelForCausalLM, BitsAndBytesConfig
            quantization_config = BitsAndBytesConfig(load_in_4bit=True)
            self.model[model_id] = AutoModelForCausalLM.from_pretrained(
                model_id,
    ...
    def stream_response(self, text_prompt, chatHistory, receiveing_tab_id, prompt_path):
        ...
        try:
            ...
            descriptions_text = "\n\n".join([f"Image {i+1}: {desc[:500]}" for i, desc in enumerate(image_descriptions)])
            

            addon=''
            if text_prompt.strip()!='':
                addon=f"Additional customisation:{text_prompt}"      
            prompt = f"""Come up with fused prompt for image generation. add intricate details. Just show one promt in <prompt> tags. use these as inspiration: 
{descriptions_text}
{addon}"""
            
            log(f'Prompt: {prompt}', 'blue')
        ...
        return ''.join(out)   
class NoHist_Flash_pChat_ResponseStreamer:
    subscribed=False
    def __init__(self):
        # Set your OpenAI API key here
        self.model={}
...
{addon}"""
        ...
        return ''.join(out)   
class NoHist_Full_pChat_ResponseStreamer:
    subscribed=False
    def __init__(self):
        # Set your OpenAI API key here
        self.model={}
...
{addon}"""
        ...
        return ''.join(out)  
class Cop_Hist_4bit_Batch_ResponseStreamer:
    subscribed=False
    def __init__(self):
        # Set your OpenAI API key here
        self.model={}
...
{addon}"""
        ...
        return ''.join(out)    
class Cop_Hist_Flash_Batch_ResponseStreamer:
    subscribed=False
    def __init__(self):
        # Set your OpenAI API key here
        self.model={}
...
{addon}"""
        ...
        return ''.join(out)   
class Cop_Hist_Full_Batch_ResponseStreamer:
    subscribed=False
    def __init__(self):
        # Set your OpenAI API key here
        self.model={}
...
{addon}"""
        ...
        return ''.join(out)   
class Chat_Hist_4bit_ResponseStreamer:
    subscribed = False
    def __init__(self):
        # Set your OpenAI API key here
        self.model = {}
    ...
    def stream_response(self, text_prompt, chatHistory, receiving_tab_id, prompt_path):
        ...
        try:
            ...
            input_text = self.prepare_input_with_history(text_prompt, chat_history,descriptions_text)
            ...
            log(f'Generate {time.time() - logits_processor[0].start}, Total: {time.time() - start}')   
    ...
    def prepare_input_with_history(self, text_prompt, chat_history, descriptions_text):
...
{addon}"""
        ...
        log(f'Prompt: {prompt}', 'blue')
        return f"{history_text}Human: {prompt}\nAI:"
...
class Chat_Hist_Flash_ResponseStreamer:
    subscribed = False
    def __init__(self):
        # Set your OpenAI API key here
        self.model = {}
...
class Chat_Hist_Full_ResponseStreamer:
    subscribed = False
    def __init__(self):
        # Set your OpenAI API key here
        self.model = {}
...
#old
class Copilot_InputPanel(wx.Panel, NewChat, GetClassName, Base_InputPanel_1):
    subscribed=False
    def __init__(self, parent, tab_id):
        ...
        Base_InputPanel_1.AddButtons_Level_1(self, askSizer)
        ...
        sizer = wx.BoxSizer(wx.VERTICAL)
        h_sizer = wx.BoxSizer(wx.HORIZONTAL)
        Base_InputPanel_1.AddButtons_Level_2(self, h_sizer)
    ...
    def get_rs(self, tab_id):
        ...
        if streamer_name not in self.rs:
            ...
            print(f'\t\Stream already exists:', streamer_name   )
...
class Chat_InputPanel(wx.Panel, NewChat,GetClassName, Base_InputPanel_1):
    def __init__(self, parent, tab_id):
        global chatHistory,  currentQuestion, currentModel
