...
with open(yaml_files[0], 'r') as file:
    all_templates = yaml.load(file, Loader=MyLoader)



pp(all_templates)
#e()
apc.default_workspace = all_templates.templates.workspace
...
all_chats=all_templates.templates.tabs.Chat + all_templates.templates.tabs.Copilot
MODEL  = 'gpt-4o'
...
class VendorNotebook(wx.Notebook):
    ...
    def OnMouseMotion(self, event):
        ...
        event.Skip()
    def GetToolTipText(self):
        return f'{apc.default_workspace.name}'
...
class MyApp(wx.App):
    def OnInit(self):
        self.frame = MyFrame(f'{apc.default_workspace.name} Copilot')
        return True
