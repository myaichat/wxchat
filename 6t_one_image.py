...
class ChooseSaveDialog(wx.Dialog):
    ...
    def save_orig(self, evt):
        save_dialog = wx.FileDialog(self, "Save Original to JPEG", "", "", "JPG files(*.jpg)", wx.FD_SAVE|wx.FD_OVERWRITE_PROMPT)
    ...
    def save_roi(self, evt):
        save_dialog = wx.FileDialog(self, "Save ROI to JPEG", "", "", "JPG files(*.jpg)", wx.FD_SAVE|wx.FD_OVERWRITE_PROMPT)
...
class ShowCapture(wx.Frame):
    def __init__(self, capture, fps=15):
        ...
        self.fps = fps
        self.start_offset_seconds = 0
        self.end_offset_seconds = 0        
        ...
        self.bmp2 = wx.Bitmap.FromBuffer(width, height, frame)
        ...
        self.ImgControl2 = statbmp.GenStaticBitmap(panel, wx.ID_ANY, self.bmp2)

        # create sliders for offset in seconds
        frame_count = int(capture.get(cv2.CAP_PROP_FRAME_COUNT))
        self.start_offset_slider = wx.Slider(panel, value=0, minValue=0, maxValue=frame_count // fps, style=wx.SL_HORIZONTAL)
        self.end_offset_slider = wx.Slider(panel, value=0, minValue=0, maxValue=frame_count // fps, style=wx.SL_HORIZONTAL| wx.SL_INVERSE)
        ...
        sizer.Add(wx.StaticText(panel, -1, 'Right-click image to reset ROI'), (1, 4), wx.DefaultSpan, wx.ALL, 5)
        self.start_slider_value_display = wx.StaticText(panel, label=f'Start (sec): '+str(self.start_offset_slider.GetValue()))
        self.end_slider_value_display = wx.StaticText(panel, label=f'End (sec): {self.end_offset_slider.GetValue()}')
        sizer.Add(self.start_slider_value_display, (0, 8), wx.DefaultSpan, wx.ALL, 5)
        sizer.Add(self.start_offset_slider, (1, 8), (1, 4), wx.EXPAND | wx.ALL, 5)
        sizer.Add(self.end_slider_value_display, (2, 8), wx.DefaultSpan, wx.ALL, 5)
        sizer.Add(self.end_offset_slider, (3, 8), (1, 4), wx.EXPAND | wx.ALL, 5)

        tool_button_sizer = wx.BoxSizer(wx.HORIZONTAL)
        tool_button_sizer.Add(save_button, 0)
        tool_button_sizer.Add(settings_button, 0)
        sizer.Add(tool_button_sizer, (0, 7), wx.DefaultSpan, wx.ALIGN_RIGHT)
        ...
        # add image widget to the sizer grid
        sizer.Add(self.ImgControl2, (3, 0), (1, 4), wx.EXPAND|wx.CENTER|wx.LEFT|wx.BOTTOM, 5)
        ...
        # start a timer that's handler grabs a new frame and updates the image widget
        ...
        # bind slider event
        self.start_offset_slider.Bind(wx.EVT_SLIDER, self.on_start_offset_change)
        self.end_offset_slider.Bind(wx.EVT_SLIDER, self.on_end_offset_change)
    ...
    def on_start_offset_change(self, evt):
        self.start_offset_seconds = self.start_offset_slider.GetValue()
        self.start_slider_value_display.SetLabel(f'Start (sec): '+str(self.start_offset_slider.GetValue()))
        self.capture.set(cv2.CAP_PROP_POS_FRAMES, self.start_offset_seconds * self.fps)

    def on_end_offset_change(self, evt):
        self.end_offset_seconds = self.end_offset_slider.GetValue()
        self.end_slider_value_display.SetLabel(f'End (sec): {self.end_offset_seconds}')

    def NextFrame(self, event):
        ret, self.orig_frame = self.capture.read()
        current_frame = self.capture.get(cv2.CAP_PROP_POS_FRAMES)
        end_frame = (self.capture.get(cv2.CAP_PROP_FRAME_COUNT) - self.end_offset_seconds * self.fps)
        ...
        if ret:
            ...
            try:
                ...
                if y1 < y2 and x1 < x2:
                    ...
                    frameRoi = cv2.cvtColor(self.frameRoi, cv2.COLOR_BGR2RGB)
                    self.bmp2 = wx.Bitmap.FromBuffer(roi_width, roi_height, frameRoi)
                    self.ImgControl2.SetBitmap(self.bmp2)
