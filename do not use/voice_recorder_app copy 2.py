...
if st.session_state.transcription and not st.session_state.recording:
    ...
    col_chatgpt, col_stop = st.columns([2, 1])
    
    with col_chatgpt:
        if st.button("💬 Get ChatGPT Response", disabled=st.session_state.generating_response):
            st.session_state.generating_response = True
            st.session_state.chatgpt_response = None
            st.session_state.stop_streaming = False  # Reset stop streaming flag
            
            # Get the current text from the transcription input field (user may have edited it)
            edited_transcription = st.session_state.get("transcription_display", st.session_state.transcription)
            st.session_state.manual_transcription = edited_transcription  # Store for manual processing
            
            st.rerun()  # Refresh to show generating status and enable stop button
    
    with col_stop:
        if st.button("🛑 Stop Streaming", disabled=not st.session_state.generating_response):
