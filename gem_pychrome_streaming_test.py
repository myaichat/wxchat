...
def send_message_with_streaming(question, timeout=60):
    ...
    try:
        ...
        while time.time() - start_time < timeout:
            ...
            # Only show updates when text actually changes
            if current_text != previous_text and current_length > previous_length:
                update_count += 1
                
                # Show the new content that was added
                new_content = current_text[previous_length:] if previous_length > 0 else current_text[:100]
                
                print(f"\n🔄 Update #{update_count} - Added {current_length - previous_length} chars")
                print(f"New content: {new_content}...")
                print(f"Total: {len(current_text.split())} words, {current_length} chars")
                print("=" * 50)
        ...
        # Get final response and display it properly
        final_text = get_response_text(tab, response_selector, response_index)
        if final_text:
            print_streaming_response(final_text, is_complete=True)
            return final_text
        else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        answer = send_message_with_streaming(args.question, timeout=args.timeout)
