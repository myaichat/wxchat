...
class ShowCapture(wx.Frame):
    def __init__(self, capture, fps=15):
        ...
        self.start_offset_slider = wx.Slider(panel, value=0, minValue=0, maxValue=int(capture.get(cv2.CAP_PROP_FRAME_COUNT) / fps), style=wx.SL_HORIZONTAL)
        ...
        # bind slider event
        self.start_offset_slider.Bind(wx.EVT_SLIDER, self.on_offset_change)
    ...
    def on_offset_change(self, evt):
        self.offset_seconds = self.start_offset_slider.GetValue()
        self.start_slider_value_display.SetLabel(f'Start (sec): '+str(self.start_offset_slider.GetValue()))
        self.capture.set(cv2.CAP_PROP_POS_FRAMES, self.offset_seconds * self.fps)
