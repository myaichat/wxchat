...
class MainFrame(wx.Frame):
    def __init__(self):
       ...
       # Create dropdown list for model name
        self.model_choice = wx.Choice(self.panel, choices=["gpt-4o", "gpt-4-turbo"])
        self.model_choice.SetSelection(0)  # Select the first model by default


        describe_image_btn = wx.Button(panel, label="Mix it up!")
        describe_image_btn.Bind(wx.EVT_BUTTON, self.on_describe_image)

        button_sizer.Add(self.model_choice, 0, wx.ALL, 5)
        button_sizer.Add(describe_image_btn, 1, wx.ALL, 5)

        question_sizer.Add(button_sizer, 0, wx.ALL | wx.CENTER, 5)
        #self.question_input.SetValue("Mix descriptions of these images into one creative description. add depth and emotions..")
        self.question_input.SetValue('merge descriptions of these images into one creative description. add depth and emotions. add  ukrainian essence. add blue and yellow theme.')
    ...
    def on_describe_image(self, event):
        if hasattr(self, 'base64_image') and len(self.base64_image)==2:
            ...
            try:
                if 1:
                    for i in range(100):  # This is just an example, replace it with your actual task
                        wx.MilliSleep(5)  # Simulate long operation
        ...
        payload = {
            ...
            "messages": [
                ...
                "content": [
                    ...
                    "text": self.question_input.GetValue(),
                    },
                    {
                    "type": "image_url",
                    "image_url": {
