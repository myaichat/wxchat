...
class MainFrame(wx.Frame):
    def __init__(self, size=(800, 600)):
        ...
        if 0:
            ...
            # Create a wx.grid.Grid
            self.answer_output = gridlib.Grid(panel)
            self.answer_output.CreateGrid(0, 4)  # Create a grid with 0 rows and 4 columns

            # Set column labels
            self.answer_output.SetColLabelValue(0, "Line Num")
            self.answer_output.SetColLabelValue(1, "Elapsed Sec")
            self.answer_output.SetColLabelValue(2, "Model Name")
            self.answer_output.SetColLabelValue(3, "Text")
            self.answer_output.SetColSize(3, 600)
            if 1:
                for i in range(5):
                    self.answer_output.AppendRows(1)
                    self.answer_output.SetCellValue(i, 0, "Short text")
                    self.answer_output.SetCellValue(i, 1, "Short text")
                    self.answer_output.SetCellValue(i, 2, "Short text")
                    self.answer_output.SetCellValue(i, 3, "This is a longer piece of text that should expand the row \nheight significantly to fit all content.")
        ...
        sizer.Add(self.answer_output, 1, wx.EXPAND)
        ...
        if 1:
            ...
            p_sizer.Add(panel, 1, wx.EXPAND|wx.ALL)
    ...
    def on_resize(self, event):
        # Resize the column
        #self.answer_output.AutoSizeColumn(3)
        #self.answer_output.SetColSize(3, 600)
        #self.Layout()
        #self.answer_output.SetColSize(3, 600)
        # Skip the event
    ...
    def _on_resize(self, event):
        ...
        column_count = self.answer_output.GetNumberCols()

        # Get the number of rows
        row_count = self.answer_output.GetNumberRows()

        total_width = sum(self.answer_output.GetColumnWidth(i) for i in range(column_count - 1))
