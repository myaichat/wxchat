...
from include.Gpt4_Python import Gpt4_Chat_InputPanel, Gpt4_Chat_DisplayPanel, Gpt4_Copilot_DisplayPanel
...
class MyNotebookImagePanel(wx.Panel):
    def __init__(self, parent, tab_id):
        ...
        if 'default_file' in chat:
            ...
            print(fn)
            self.DisplayImageOnCanvas(fn)
            notebook.AddPage(self.canvasCtrl, fn)
        else:
    ...
    def DisplayImageOnCanvas(self, image_path):
        ...
        # Get the canvas size
        canvas_width, canvas_height = self.canvasCtrl.GetSize()
        ...
        # Resize the image
        image = image.Scale(int(image_width/2), int(image_height/2), wx.IMAGE_QUALITY_HIGH)
    ...
    def _DisplayImageOnCanvas(self, image_path):
        # Load the image
        image = wx.Image(image_path, wx.BITMAP_TYPE_ANY)
        # Convert it to a bitmap
        bitmap = wx.Bitmap(image)
