                                ...
                                if (node.nodeType === Node.ELEMENT_NODE && !responseFound) {{
                                    ...
                                    if (isAssistantMessage(node)) {{
                                        ...
                                        const responseObserver = new MutationObserver(function(responseMutations) {{
                                            ...
                                            if (!responseComplete && currentContent.length > 50 && checkIfComplete(responseElement)) {{
                                        ...
                                        const completionCheck = setInterval(() => {{
                                            ...
                                            if (currentContent === lastResponseText && currentContent.length > 50) {{
                                                ...
                                                // Increased from 4 to 10 checks (5 seconds instead of 2)
                                                if (stableTextCount >= 10) {{
                                                    // Additional validation before marking complete
                                                    if (checkIfComplete(responseElement)) {{
                                                        responseComplete = true;
                                                        console.log("RESPONSE_COMPLETE:" + currentContent);
                                                        responseObserver.disconnect();
                                                        clearInterval(completionCheck);
                                                        ...
                                                        // Reset counter if completion indicators not found
                                                        console.log("COMPLETION_DELAYED: No completion indicators found, continuing to wait");
                                                        stableTextCount = 0;
                                                    }}
                                                }}
                                            }} else {{
