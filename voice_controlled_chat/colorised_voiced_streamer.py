    ...
    for chunk in response:
        if hasattr(chunk.choices[0].delta, 'content') and chunk.choices[0].delta.content is not None:
            ...
            while i < len(content):
                if content[i:i+2] == '**':
                ...
                elif content[i:i+3] == '```' and not inside_stars:  # Don't start coloring in red if we're inside '**'
                    if inside_backticks:
                        new_content += f"{Style.RESET_ALL}"
                        inside_backticks = False
                    else:
                    ...
                    if content[i:i+6] == 'python':  # If 'python' follows '```', skip it
                        i += 6
                        while i < len(content) and content[i] in (' ', '\n'):  # Skip spaces and newlines after 'python'
                            i += 1
