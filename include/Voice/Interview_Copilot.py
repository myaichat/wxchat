...
from include.Voice.Base.Base_InputPanel_Interview_Copilot import Base_InputPanel_Interview_Copilot
...
class Record_ResponseStreamer:
    ...
    def stream_response(self, text_prompt, chatHistory, receiveing_tab_id, model):
        ...
        if 0:
            ...
            pub.sendMessage('chat_output', message=f'{header}\n', tab_id=receiveing_tab_id)
        start = time.time()
        from include.Voice.Streamer.Interview_Copilot import stream_response 
        #conversation(receiveing_tab_id)
        model_response, tell_me_more =stream_response(receiveing_tab_id)
...
class StyledTextDisplay(stc.StyledTextCtrl, GetClassName, NewChat, Scroll_Handlet):
    def __init__(self, parent):
        ...
        if 0:
            ...
            self.StyleSetSpec(stc.STC_P_TRIPLE, "fore:#FFA500,back:#FFFFFF,weight:bold")  # Triple quotes
            self.StyleSetSpec(stc.STC_P_TRIPLEDOUBLE, "fore:#FFA500,back:#FFFFFF")  # Triple double quotes
        ...
        self.SetStyles()
        self.SetLineBackground(3, wx.Colour(255, 255, 0))
    def SetLineBackground(self, line, color):
        marker_number = 0  # Marker number (0-31)
        self.control.MarkerDefine(marker_number, stc.STC_MARK_BACKGROUND, background=color)
        self.control.MarkerSetBackground(marker_number, color)
        self.control.MarkerAdd(line - 1, marker_number) 

    def SetStyles(self):
        # Use a monospace font
        base_font = "face:Consolas,Courier New,Monospace,size:10"
        self.control.StyleSetSpec(stc.STC_STYLE_DEFAULT, base_font)
        self.control.StyleClearAll()
        self.control.StyleSetSpec(1, f"{base_font},fore:#800000,bold")  # Bold style
        self.control.StyleSetSpec(2, f"{base_font},fore:#008080,back:#F0F0F0")  # Code block style
        self.control.StyleSetSpec(3, f"{base_font},fore:#0000FF,bold,size:14")  # H1 style
        self.control.StyleSetSpec(4, f"{base_font},fore:#0000FF,bold,size:13")  # H2 style
        self.control.StyleSetSpec(5, f"{base_font},fore:#0000FF,bold,size:12")  # H3 style
        self.control.StyleSetSpec(6, f"{base_font},fore:#0000FF,bold,size:11")  # H4 style
        self.new_line_style = 1
        self.control.StyleSetBackground(self.new_line_style, wx.Colour(255, 255, 0))

    def on_style_needed(self, event):
        start_pos = self.control.GetEndStyled()
        end_pos = event.GetPosition()
        text = self.control.GetTextRange(0, end_pos)
        
        # Reset all styling
        self.control.StartStyling(0)
        self.control.SetStyling(end_pos, 0)
        ...
        for line in lines:
            if line.strip().startswith('```'):
                if inside_code_block:
                    self.control.StartStyling(code_block_start)
                    self.control.SetStyling(pos + len(line) - code_block_start, 2)
                    inside_code_block = False
                else:
                ...
                if line.startswith('####'):
                    ...
                    self.control.SetStyling(len(line), 6)
                elif line.startswith('###'):
                    self.control.StartStyling(pos)
                    self.control.SetStyling(len(line), 5)
                elif line.startswith('##'):
                    self.control.StartStyling(pos)
                    self.control.SetStyling(len(line), 4)
                elif line.startswith('#'):
                    self.control.StartStyling(pos)
                    self.control.SetStyling(len(line), 3)
                else:
                    # Check for bold within the line
                    i = 0
                    while i < len(line):
                        if line[i:i+2] == '**':
                            end = line.find('**', i+2)
                            ...
                            self.control.StartStyling(pos + i)
                            self.control.SetStyling(end - i, 1)
                            i = end
                        else:
                            i += 1
            pos += len(line) + 1  # +1 for the newline character

        # Handle case where code block extends to the end of the text
        if inside_code_block:
            self.control.StartStyling(code_block_start)
            self.control.SetStyling(pos - code_block_start, 2)
    ...
    def _on_style_needed(self, event):
        ...
        # Find and style bold text and code blocks
        inside_bold = False
        inside_code = False
        bold_start = code_start = 0
        i = 0
        while i < len(text):
            if text[i:i+3] == '```':
                if inside_code:
                    self.control.StartStyling(code_start)
                    self.control.SetStyling(i - code_start + 3, 2)  # +3 to include ```
                    inside_code = False
                    i += 3
                else:
                    code_start = i
                    inside_code = True
                    i += 3
            elif text[i:i+2] == '**' and not inside_code:
                if inside_bold:
                    self.control.StartStyling(bold_start)
                    self.control.SetStyling(i - bold_start + 2, 1)  # +2 to include **
                    inside_bold = False
                    i += 2
                else:
                    bold_start = i
...
#old
class Copilot_InputPanel(wx.Panel, NewChat, GetClassName, Base_InputPanel_Interview_Copilot):
    subscribed=False
    def __init__(self, parent, tab_id):
        ...
        Base_InputPanel_Interview_Copilot.AddButtons_Level_1(self, askSizer)
        ...
        sizer = wx.BoxSizer(wx.VERTICAL)
        h_sizer = wx.BoxSizer(wx.HORIZONTAL)
        Base_InputPanel_Interview_Copilot.AddButtons_Level_2(self, h_sizer)
...
class MyDialog(wx.Dialog):
    def __init__(self, parent, title):
        super(MyDialog, self).__init__(parent, title=title, size=(200, 80))
    ...
    def InitUI(self):
        ...
        self.ask_button = wx.Button(panel, label='Ask')

        vbox.Add(self.record_button, proportion=0, flag=wx.ALL | wx.CENTER, border=10)
        vbox.Add(self.ask_button, proportion=0, flag=wx.ALL | wx.CENTER, border=10)

        panel.SetSizer(vbox)

        self.Bind(wx.EVT_BUTTON, self.OnRecord, self.record_button)
        self.Bind(wx.EVT_BUTTON, self.OnAsk, self.ask_button)
    ...
    def OnAsk(self, event):
        #wx.MessageBox("Ask button clicked!", "Info", wx.OK | wx.ICON_INFORMATION)
        log('Asking...')
        
        self.chat.record=False
        self.EndModal(wx.ID_OK)

    def OnKeyDown(self, event):
        keycode = event.GetKeyCode()
        if keycode == wx.WXK_RETURN:
            if self.record_button.HasFocus():
                print('keydown')
                self.OnRecord(None)
            else:
                event.Skip()
...
class Chat_InputPanel(wx.Panel, NewChat,GetClassName, Base_InputPanel_Interview_Copilot):
    def __init__(self, parent, tab_id):
        global chatHistory,  currentQuestion, currentModel
    ...
    def get_rs(self, tab_id):
        chat=apc.chats[tab_id]
        if chat.record:
            streamer_name = f'{chat.record_streamer_name}_ResponseStreamer'
        else:
            streamer_name = f'{chat.ask_streamer_name}_ResponseStreamer'
        if streamer_name not in self.rs:
    ...
    def OnCharHook(self, event):
        if event.ControlDown() and  event.GetKeyCode() == wx.WXK_RETURN:
            #open dialog to select Record vs Ask
            dlg = MyDialog(None, title="Record or Ask?")
            dlg.chat= apc.chats[self.tab_id]
            dlg.CentreOnParent()
            dlg.ShowModal()
            
            dlg.Destroy()
            self.AskQuestion()
        #add option ControlDown + letter 'A'
