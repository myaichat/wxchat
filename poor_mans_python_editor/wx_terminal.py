...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        if 1:
            ...
            self.button = wx.Button(self.button_panel, -1, 'Run', pos=(0, 0))
            self.button.SetBackgroundColour(wx.Colour(200, 200, 200))
            self.button.Bind(wx.EVT_BUTTON, self.on_execute_script)
    ...
    def on_execute_script(self, event):
        if 1:

            command = 'start cmd /k "python --version"'
    ...
    def _on_execute_script(self, event):
